...
def loadmodel(path):
    ...
    return model
'''
quesNo = 2  # to mark the question number, for data saving
...
model = load_model(saveDir+'%s_cifar10_model.h5' %(str(quesNo-1)))
model.summary()
...
t1 = float(time.clock())
print ('Data Loading time is %.4f s.' % (t1-t0))
t0 = t1
'''
model = Sequential()
...
#model.load_weights(load_path+'.h5')  # loads a HDF5 file '*-1_cifar10_model.h5' for weights
'''
#model = Sequential(L_model.layers)
print('before pop    ' ,len(model.layers))
layer1 = model.layers.pop()
layer2 = model.layers.pop()
...
print('after pop')
model.summary()
model.add(Dense(512,name="dense_new"))
model.add(Activation('relu',name="act_new"))
model.add(Dense(nb_classes,name="dense_10"))
model.add(Activation('softmax',name="new_soft"))


# print('after add      ', len(model.layers))
print('after add all')
model.summary()
t1 = float(time.clock())
