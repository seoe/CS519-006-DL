...
def loadnwrite():
    path = 'TestAcc_tune_lr+hidden_100+mb_1000.csv'
    mat = np.genfromtxt(path, delimiter = ',')
    line, epoch = mat.shape

    color = 0
    label_list = [0.005,0.001,0.0005,0.0001]

    plt.figure()
    for i in xrange(line):
        plt.plot(range(1,epoch+1),mat[i], get_colour(color),label="%s %s" %('learning_rate ',str(label_list[i])))
        plt.xlim(0, epoch)
        plt.ylim(0.6, 0.9)
        plt.xlabel('epoch')
