...
def loadnwrite():
    path = 'TestAcc_tune_hidden+lr_0.001+mb_1000.csv'
    mat = np.genfromtxt(path, delimiter = ',')
    line, epoch = mat.shape

    color = 0
    label_list = [10,100,1000]

    plt.figure()
    for i in xrange(line):
        plt.plot(range(1,epoch+1),mat[i], get_colour(color),label="%s %s" %('hidden_units ',str(label_list[i])))
        plt.xlim(0, epoch)
        plt.ylim(0.6, 0.9)
        plt.xlabel('epoch')
